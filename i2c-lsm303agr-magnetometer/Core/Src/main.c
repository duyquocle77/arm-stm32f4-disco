...
main() {
	...
	while (1) {
		...
		/*cfg acc mode*/
		i2c_write_data(MAG_ADDRESS_7BIT, 0x60, 0x8C);
		i2c_write_data(MAG_ADDRESS_7BIT, 0x61, 0x02);
		i2c_write_data(MAG_ADDRESS_7BIT, 0x62, 0x10);

		/*wait Zyxda bit*/
		while (((i2c_read_data(MAG_ADDRESS_7BIT, 0x67) >> 3) & 1) == 0);
		/*read magnet value*/
		x_l = i2c_read_data(MAG_ADDRESS_7BIT, 0x68);
		x_h = i2c_read_data(MAG_ADDRESS_7BIT, 0x69);
		y_l = i2c_read_data(MAG_ADDRESS_7BIT, 0x6A);
		y_h = i2c_read_data(MAG_ADDRESS_7BIT, 0x6B);
		z_l = i2c_read_data(MAG_ADDRESS_7BIT, 0x6C);
		z_h = i2c_read_data(MAG_ADDRESS_7BIT, 0x6D);
